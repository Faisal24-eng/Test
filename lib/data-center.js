...
class DataCenter {
    constructor() {
        ...
        this.wsClients = {
            1: new BinanceAPI.BinanceWebSocketClient(),
            2: new BinanceAPI.BinanceWebSocketClient()
        };
    ...
    getWsStatus() {
        return {
            1: this.wsClients[1] ? this.wsClients[1].isConnected : false,
            2: this.wsClients[2] ? this.wsClients[2].isConnected : false
        };
    }
    ...
    /**
     * Start data fetching
     * Perform initial fetch, then open WebSockets for real-time updates.
     */
    async start() {
        // Connect WebSockets IMMEDIATELY so we don't wait for slow HTTP requests!
        if (this.markets[1].config) this.connectWebSocket(1);
        if (this.markets[2].config) this.connectWebSocket(2);

        // Just do one initial fetch to pad history - DO NOT AWAIT to avoid blocking WS!
        this.fetchOneRound().catch(err => console.error('[DataCenter] Initial fetch error:', err));
    }

    /**
     * Connect WebSocket for a specific market
     */
    connectWebSocket(marketNum) {
        const market = this.markets[marketNum];
        const config = market.config;
        const ws = this.wsClients[marketNum];

        // Ensure we have config
        if (!config || !config.ohlcv) {
            console.log(`[DataCenter] WS connect skipped for market ${marketNum} - no config`);
            return;
        }

        console.log(`[DataCenter] Initiating WS connect for market ${marketNum}: ${config.ohlcv.symbol} @ ${config.ohlcv.interval} (${config.ohlcv.marketType})`);

        ws.onKline = (kline) => {
            this.updateRollingOHLCV(marketNum, [kline]);
            market.lastOhlcvFetch = Date.now();
            this.notify('ohlcv', marketNum, market.ohlcv);
            
            // Critical Event: Emit candle close for instant trade execution
            if (kline.isClosed) {
                this.notify('candleClosed', marketNum, kline);
            }
        };
        ...
        ws.connect(config.ohlcv.symbol, config.ohlcv.interval, config.ohlcv.marketType);
    }

    /**
     * Stop data fetching and disconnect WebSockets
     */
    stop() {
        this.wsClients[1].disconnect();
        this.wsClients[2].disconnect();
    }
    ...
    async fetchOHLCV(marketNum, retryCount = 0) {
        ...
        // If WebSocket is active, we skip the REST API request to save bandwidth and rate limits.
        if (!isFirstFetch && market.lastOhlcvFetch) {
            const timeSinceLastWSUpdate = Date.now() - market.lastOhlcvFetch;
            // If data is fresher than 15 seconds, WS is healthy. Skip REST.
            if (timeSinceLastWSUpdate < 15000) { 
                console.log(`[DataCenter] Skipping REST OHLCV fetch for market ${marketNum} (WebSocket data is fresh)`);
                return;
            }
        }
