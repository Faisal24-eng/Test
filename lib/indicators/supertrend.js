...
function getSupertrendSignal(ohlcv, atrPeriod = 7, multiplier = 2, source = 'close') {
    ...
    // Improved logic: trend changed if color flips, 
    // OR if we were in neutral (0) and now have a clear direction (1 or -1)
    const trendChanged = (currentTrend !== previousTrend && previousTrend !== 0) || (previousTrend === 0 && currentTrend !== 0);
