...
// AI Indicator import
import { getAISignal, refreshAIContext } from './indicators/ai.js';
...
class SignalEngine {
    constructor() {
        ...
        this.indicatorWeights = {
            ...
            ai: 10                   // AI composite indicator (highest weight)
        };
    ...
    processSignals(data, config, currentPosition) {
        ...
        // AI Indicator
        this.processAISignal(signals, market1OHLCV, market2OHLCV, config, currentPosition);
     ...
     * Process AI Signal
     */
    processAISignal(signals, market1OHLCV, market2OHLCV, config, currentPosition) {
        const cfg = config.ai;
        if (!cfg || cfg.enabled === 'off') return;
        ...
        const signal = getAISignal(resampledOHLCV, cfg);
        signals.details.ai = signal;

        // Primary signal
        if (cfg.enabled === 'primary') {
            if (signal.primaryLong) {
                signals.primary = 'long';
            } else if (signal.primaryShort) {
                signals.primary = 'short';
            }
        }
