...
class Backtester {
    ...
    runSimulation(indicatorConfig, backtestConfig, onProgress) {
        ...
        // Ensure isRunning is true (optimizer calls this directly, not via run())
        this.isRunning = true;

        // Reset signal engine state to prevent leakage between runs (critical for optimizer)
        this.signalEngine.reset();
        // Preserve backtestMode flag (set by optimizer or run() method)
        this.signalEngine.backtestMode = true;
        ...
        // We need enough candles to satisfy the largest timeframe and period
        let maxTimeframe = 1;
        Object.values(indicatorConfig).forEach(cfg => {
            if (cfg && cfg.enabled !== 'off' && cfg.timeframe) {
                maxTimeframe = Math.max(maxTimeframe, cfg.timeframe);
            }
        });
        
        // Ensure at least 40 candles of the largest timeframe for indicator warm-up
        const lookbackPeriod = maxTimeframe * 40; 
        console.log(`[Backtester] Max timeframe: ${maxTimeframe}m. Using dynamic lookback: ${lookbackPeriod} candles`);
        
        const executionData = executionMarket === 2 ? market2Data : market1Data;
        if (!executionData || executionData.length <= lookbackPeriod) {
            console.warn('[Backtester] Not enough data for simulation after lookback.');
    ...
    prepareBacktestConfig(config) {
        ...
        // Configure AI for backtest mode:
        // - Skip safety guards (cooldown, session, funding, liquidation filters)
        //   These all depend on live API data and Date.now() which are meaningless in simulation
        // - The confluence engine and regime detector still work from OHLCV data
        if (backtestConfig.ai) {
            backtestConfig.ai.backtestMode = true;
            backtestConfig.ai.sessionFilter = false;
            backtestConfig.ai.fundingFilter = false;
            backtestConfig.ai.liquidationFilter = false;
        }
    ...
    /**
     * Return empty statistics object (when not enough data for simulation)
     */
    getEmptyStats() {
        return {
            summary: {
            ...
            advancedMetrics: {
                ...
                calmarRatio: '0.00',
                expectancy: '0.00',
                expectancyPercent: '0.00',
                avgTradeDuration: '0.0 min',
                tradesPerDay: '0.00',
                avgRR: '0.00',
                holdingPeriodDays: '0.0'
            },
            breakdown: {
                longTrades: 0,
                longWins: 0,
