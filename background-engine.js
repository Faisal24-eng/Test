...
import { refreshAIContext, getAISignalAsync } from './lib/indicators/ai.js';
...
class BackgroundService {
    ...
    async handleMessage(message, sender, sendResponse) {
        try {
            switch (message.action) {
                ...
                case 'refreshAIContext':
                    const symbol = message.symbol || (this.config?.market1?.ohlcv?.symbol || 'BTCUSDT');
                    const context = await refreshAIContext(symbol);
    ...
    getPublicState() {
        return {
            ...
            wsStatus: dataCenter.getWsStatus ? dataCenter.getWsStatus() : 'OFF'
        };
    }
    ...
    async checkSignals() {
        ...
        try {
            ...
            // Refresh AI context before processing signals
            const aiSymbol = this.config?.market1?.ohlcv?.symbol || 'BTCUSDT';
            await refreshAIContext(aiSymbol);

            // Process signals
            const signals = await signalEngine.processSignals(
                data,
